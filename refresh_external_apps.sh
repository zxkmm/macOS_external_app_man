...
# recursively sync directories and create symlinks
sync_recursive() {
    local source_path="$1"
    local target_path="$2"
    ...
    while IFS= read -r item_path; do
        ...
        if [ -L "$item_path" ]; then
            # it's a symlink, create symlink to the symlink itself
            if [ -L "$target_item_path" ]; then
                ...
                if [ "$current_target" = "$item_path" ]; then
                    echo "skip $relative_item_path (symlink already exists)"
                    ((skipped_count++))
                else
                    ...
                    if [ $? -eq 0 ]; then
                        ...
                        echo "failed updating symlink for $relative_item_path"
                    fi
                fi
            elif [ -e "$target_item_path" ]; then
                echo "skip $relative_item_path (exists but not a symlink)"
                ((skipped_count++))
            else
                ln -s "$item_path" "$target_item_path"
                if [ $? -eq 0 ]; then
                    echo "linked symlink $relative_item_path"
                    ((linked_count++))
                else
                    echo "failed creating symlink for $relative_item_path"
                fi
            fi
        elif [ -d "$item_path" ] && [[ "$item_name" != *.app ]]; then
    ...
    done < <(find "$source_path" -maxdepth 1 -mindepth 1 \( -type f -o -type d -o -type l \) | sort)
}

# recursively clean up dead symlinks
cleanup_recursive() {
    local target_path="$1"
